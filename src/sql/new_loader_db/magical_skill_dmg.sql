...
--upper(substr(skills.name, 1, 4)) class, 
skills.name,
--upper(substr(skills.name, 1, 3)) classCode,
learn.class class_learn, 

case when str_NA.body = str_EU.body 
    then coalesce(str_NA.body, skills.desc) || case when chain_category_level is not null then ' Lv ' || chain_category_level else '' end 
...
effect1_critical_prob_mod2 crit_prob_mod,
...
--and (upper(substr(skills.name, 1, 3)) in ('MA_','WI_') or class_learn in ('MAGE','WIZARD'))
and (upper(substr(skills.name, 1, 3)) in ('EL_') or class_learn in ('MAGE','ELEMENTALLIST'))
--and (upper(substr(skills.name, 1, 3)) in ('CL_','PR_') or class_learn in ('CLERIC','PRIEST'))
--and (upper(substr(skills.name, 1, 3)) in ('CL_','CH_') or class_learn in ('CLERIC','CHANTER'))
...
--and (upper(substr(skills.name, 1, 3)) in ('EN_','GU_') or class_learn in ('ENGINEER','GUNNER'))
--and (upper(substr(skills.name, 1, 3)) in ('EN_','RI_') or class_learn in ('ENGINEER','RIDER'))
