...
--like '%SpellATK%_Instant' --body = 'Break Power'

and (upper(substr(skills.name, 1, 3)) in ('MA_','WI_') or class_learn in ('MAGE','WIZARD')) and coalesce(class_learn, '') <> ('ELEMENTALLIST')
--and (upper(substr(skills.name, 1, 3)) in ('EL_') or class_learn in ('MAGE','ELEMENTALLIST')) and coalesce(class_learn, '') <> ('MAGE')
--and (upper(substr(skills.name, 1, 3)) in ('CL_','PR_') or class_learn in ('CLERIC','PRIEST')) and coalesce(class_learn, '') <> ('CHANTER')
--and (upper(substr(skills.name, 1, 3)) in ('CL_','CH_') or class_learn in ('CLERIC','CHANTER')) and coalesce(class_learn, '') <> ('PRIEST')
--and (upper(substr(skills.name, 1, 3)) = 'AR_' or class_learn in ('ARTIST','BARD'))
--and (upper(substr(skills.name, 1, 3)) in ('WA_','KN_') or class_learn in ('WARRIOR','KNIGHT'))
--and (upper(substr(skills.name, 1, 3)) in ('SC_','AS_') or class_learn in ('SCOUT','ASSASSIN'))
--and (upper(substr(skills.name, 1, 3)) in ('EN_','GU_') or class_learn in ('ENGINEER','GUNNER')) and coalesce(class_learn, '') <> ('RIDER')
--and (upper(substr(skills.name, 1, 3)) in ('EN_','RI_') or class_learn in ('ENGINEER','RIDER')) and coalesce(class_learn, '') <> ('GUNNER')
--and skill_group_name is not null

--and skills.name like 'HD%'
--and skills.name like 'HDW%' or skills.name like 'HDA%'
--and str.body like 'Electro Bolts%'
--and skills.name like 'IDTransform_HD%'
order by str.body, cast(chain_category_level as number), damage

--order by lower(element), damage desc

--select * from client_skills

--select distinct effect1_type from _SKILLS_WITH_NAME_NORMALIZED_EFFECTS
