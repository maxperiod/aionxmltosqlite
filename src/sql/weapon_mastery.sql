...
--upper(substr(skills.name, 1, 2)) class, 
--learn.class learn_class, 
case when trim(str_na.body) = trim(str_eu.body) then coalesce(str_na.body, skills.desc) || case when chain_category_level is not null then ' Lv ' || chain_category_level else '' end 
     else coalesce(str_na.body||' / '||str_eu.body, skills.desc) || case when chain_category_level is not null then ' Lv ' || chain_category_level else '' end
end skill_name,
--coalesce(str_na.body, skills.desc) || case when chain_category_level is not null then ' Lv ' || chain_category_level else '' end skill_NA,
...
--effect1_effectid,
--effect1_basiclv,
pc_level required_lvl,
effect1_reserved2 dmg_bonus_percent,
effect1_reserved5
...
and effect1_type = 'Wpn_Mastery'
and (learn.class is not null/* or upper(substr(skills.name, 1, 2)) in (
'WA',
'FI',
'KN',
'SC',
...
order by effect1_effectid, effect1_basiclv/*str_na.body, cast(chain_category_level as number), learn_class desc
