...
upper(substr(skills.name, 1, 2)) class, learn.class learn_class, coalesce(body, skills.desc) || case when chain_category_level is not null then ' Lv ' || chain_category_level else '' end skill,
effect1_type, 
...
FROM client_skills skills
left outer join
/*client_strings_skill*/ str
on upper(skills.desc) = str.name
left outer join 
...
order by body, cast(chain_category_level as number), learn_class desc
